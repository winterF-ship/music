...
export const usePlayerStore = defineStore('player', () => {
  ...
  function previous() { if (queue.value.length) { currentIndex.value = (currentIndex.value - 1 + queue.value.length) % queue.value.length; currentTime.value = 0; isPlaying.value = true } }
  function next() { if (queue.value.length) { currentIndex.value = (currentIndex.value + 1) % queue.value.length; currentTime.value = 0; isPlaying.value = true } }
  ...
  return { queue, currentIndex, currentSong, isPlaying, currentTime, duration, volume, seekVersion, playSong, playAll, playAt, toggle, previous, next, removeFromQueue, clearQueue, setVolume, seek }
})
